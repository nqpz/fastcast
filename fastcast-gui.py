...
eye_orig = Eye(position=Vec3(x=0.0, y=0.0, z=0.0),
               orientation=Vec3(x=0.0, y=0.0, z=0.0))
...
spheres_test0 = [
    # The first sphere must always be a dummy sphere.  This is picked in the
    # renderer if no other sphere matches.
    Sphere(center=Vec3(x=0.0, y=0.0, z=0.0),
           radius=0.0,
    ...
    Sphere(center=Vec3(x=0.0, y=0.0, z=2300.0),
           radius=1000.0,
           color=RGB(r=0.1, g=0.95, b=0.2)),
]
for i in range(50):
...
lights_test0 = [
    Light(position=Vec3(x=300, y=200, z=50), intensity=80000,
          color=RGB(r=0.5, g=0.5, b=0.5)), # medium grey light
    Light(position=Vec3(x=-400, y=-100, z=150), intensity=30000,
          color=RGB(r=0.8, g=0.1, b=0.1)), # small reddish light
    Light(position=Vec3(x=-500, y=-200, z=850), intensity=300000,
          color=RGB(r=0.0, g=0.15, b=0.9)), # large blueish light
]
...
light_color_rs = numpy.fromiter(map(lambda l: l.color.r, lights), dtype='float32')
light_color_gs = numpy.fromiter(map(lambda l: l.color.g, lights), dtype='float32')
light_color_bs = numpy.fromiter(map(lambda l: l.color.b, lights), dtype='float32')
...
def render():
    ...
    frame = fastcast.main(size[0], size[1], screen_view_dist,
                          ...
                          light_intensities,
                          light_color_rs, light_color_gs, light_color_bs).get()
    end = time.time()
    frame.shape = size
